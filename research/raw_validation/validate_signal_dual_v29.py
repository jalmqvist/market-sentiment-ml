...
def compute_v29(df, lookback=500, q=0.9):
    """
    V29: Conditional sentiment filter
    - Default: follow price
    - Only modify behavior when sentiment is extreme
    """

    df = df.copy()

    # ----- Price signal (unchanged core alpha)
    df["past_ret"] = df["ret_48b"].shift(48)
    mom = df["past_ret"].rolling(48).mean().shift(1)
    price = np.sign(mom)

    # ----- Sentiment residual (demeaned, causal)
    sent_mean = df["net_sentiment"].rolling(lookback).mean().shift(1)
    sent_resid = df["net_sentiment"] - sent_mean

    # ----- Rolling quantiles (causal)
    upper = sent_resid.rolling(lookback).quantile(q).shift(1)
    lower = sent_resid.rolling(lookback).quantile(1 - q).shift(1)

    # ----- Default: follow price
    signal = price.copy()

    # ----- Only act in extremes
    long_mask = (price > 0) & (sent_resid < lower)
    short_mask = (price < 0) & (sent_resid > upper)

    # reinforce price direction only when sentiment disagrees strongly
    signal[long_mask] = 1
    signal[short_mask] = -1
...
def main():
    ...
    parser.add_argument("--variants", default="baseline,price,v29")

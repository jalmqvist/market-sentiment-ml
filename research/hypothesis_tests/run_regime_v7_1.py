...
run_regime_v7_1.py
==================
Entry-point script for the Regime V7.1 continuous event scoring pipeline.

All score-building, walk-forward, and metrics logic lives in
``experiments/regime_v7_1.py``; this script is a thin launcher that configures
logging (file-only by default, no stdout) and delegates to
``experiments.regime_v7_1.main``.

Pipeline summary
----------------
Regime V7.1 upgrades V7 by replacing discrete boolean event flags with
*continuous event scores*:

1. **SATURATION_SCORE** – crowd saturation: z-scored abs_sentiment +
   extreme_streak_70 + trend_strength_48b, bounded with tanh.

2. **DIVERGENCE_SCORE** – sentiment-price divergence: |z-score of divergence|,
   bounded with tanh.

3. **EXHAUSTION_SCORE** – trend exhaustion: z(extreme_streak_70) −
   z(trend_strength_48b), bounded with tanh.

For each test row the score with the greatest absolute magnitude is chosen.
...
Usage::
    ...
    python run_regime_v7_1.py --data data/output/master_research_dataset.csv

    # Log to a specific file
    python run_regime_v7_1.py \\
        --data data/output/master_research_dataset.csv \\
        --log-file logs/regime_v7_1.log
...
def _setup_logging(level: str, log_file: str | None = None) -> None:
    ...
    Args:
        ...
        log_file: Optional explicit path to a log file.  When omitted, a
            file named ``regime_v7_1_YYYYMMDD_HHMMSS.log`` is created inside
            ``logs/``.
    """
...
def main(argv=None) -> None:
    ...
    from experiments.regime_v7_1 import (  # noqa: PLC0415
        DEFAULT_MIN_N,
    ...
    require_columns(df, _REQUIRED_COLS, context="run_regime_v7_1")

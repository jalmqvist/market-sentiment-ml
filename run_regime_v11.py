...
run_regime_v11.py
=================
Entry-point script for the Regime V11 context-aware event-ranking pipeline.

All feature loading, event detection, scoring, walk-forward, and metrics
logic lives in ``experiments/regime_v11.py``; this script is a thin launcher
that configures logging (file-only by default, no stdout) and delegates to
``experiments.regime_v11.main``.

Pipeline summary
----------------
Regime V11 upgrades V10 with **context-aware event ranking**:
...
2. **Non-linear event score** (inherited from V10)::
...
4. **Context-aware ranking + selection** — within each test fold, events are
   grouped by context key; each group is ranked independently (descending
   score) and only the top ``top_frac`` fraction per group is traded.
   Context groups with fewer than ``min_context_events`` (default 30) events
...
6. **Walk-forward** — expanding window; minimum 2 prior years before the first
   test year.

7. **Metrics per fold**: ``n_total_events``, ``n_selected_events``,
...
Usage::
    ...
    python run_regime_v11.py --data data/output/master_research_dataset.csv

    # Log to a specific file
    python run_regime_v11.py \\
        --data data/output/master_research_dataset.csv \\
        --log-file logs/regime_v11.log

    # Custom top-fraction and minimum context events
    python run_regime_v11.py \\
        --data data/output/master_research_dataset.csv \\
        --top-frac 0.3 \\
...
def _setup_logging(level: str, log_file: str | None = None) -> None:
    ...
    Args:
        ...
        log_file: Optional explicit path to a log file.  When omitted, a
            file named ``regime_v11_YYYYMMDD_HHMMSS.log`` is created inside
            ``logs/``.
    """
...
def main(argv=None) -> None:
    p = argparse.ArgumentParser(
        description=(
            ...
            "(abs_sentiment >= 70 AND extreme_streak_70 >= 2), assigns each a "
            "context key (vol × trend × sentiment regime), and ranks events "
    ...
    p.add_argument(
        ...
        help=(
            ...
            f"used. Default: {_DEFAULT_MIN_CONTEXT_EVENTS}."
        ),
    )
    p.add_argument(
        "--log-level",
    ...
    from experiments.regime_v11 import (  # noqa: PLC0415
        _REQUIRED_COLS,
    ...
    _log.info("=== REGIME V11 (CONTEXT-AWARE EVENT RANKING) ===")
    _log.info(
        "top_frac=%.4f | min_context_events=%d",
        args.top_frac,
    ...
    require_columns(df, _REQUIRED_COLS, context="run_regime_v11")

"""
experiments/regime_v7_2.py
==========================
INTERACTION-BASED SCORING pipeline for FX sentiment research.

Upgrades Regime V7.1 by replacing additive score definitions with
**multiplicative interactions** to correctly model nonlinear behavioural
effects.

Pipeline overview
-----------------
1. **Scores** – three interaction-based continuous scores per row (z-scores
   fitted on the training split only, then multiplied):
...
2. **Row-wise normalisation** (per test row)::

       norm   = sum(abs(scores)) + 1e-6
       scores = scores / norm

3. **Train phase** (per fold)
...
4. **Test phase** – for each test row:

   * Compute all scores (using train-only normalisation stats).
   * Apply row-wise normalisation.
   * ``best_idx  = argmax(|SATURATION|, |DIVERGENCE|, |EXHAUSTION|)``
   * ``best_score = scores[best_idx]``
   * If ``|best_score| < score_threshold`` → ``position = 0``.
   * Otherwise:
       ``base_signal = tanh(signal_v2_raw)``
       ``position    = base_signal * sign(best_score)``
     or, with ``--use-score-weighting``:
       ``position    = base_signal * best_score``

5. **Metrics** – coverage, n, mean, Sharpe, hit_rate, avg_score_magnitude,
   plus per-score-type performance.

6. **Walk-forward** – expanding window, minimum 3 years, train on all prior
...
* Distribution of score magnitudes (mean, std, min, max)
* Frequency of each selected score type in active rows

Usage::

    python experiments/regime_v7_2.py \\
...
def load_data(path: str | Path, *, window: int = DEFAULT_WINDOW) -> pd.DataFrame:
    """Load and prepare the dataset for Regime V7.2.
    ...
    if selected_col is None:
        raise ValueError(
            "Regime V7.2: no valid numeric price column found among "
            f"{_PRICE_CANDIDATES}."
        )
...
def build_scores(
    ...
    """Compute interaction-based continuous event scores for every row in *df*.

    Scores are derived from products of z-scores of raw features (using
    train-only mean/std to avoid leakage) and then bounded with ``tanh``.
...
def normalize_scores(scores: pd.DataFrame) -> pd.DataFrame:
    """Apply row-wise L1 normalisation to the score matrix.
    ...
    This ensures scores are on a comparable scale regardless of how extreme
    the individual z-score products are.

    Args:
        scores: DataFrame with columns ``_SCORE_NAMES``.

    Returns:
        New DataFrame of the same shape with row-normalised scores.
    """
    arr = scores[_SCORE_NAMES].values.astype(float)
    norm = np.abs(arr).sum(axis=1, keepdims=True) + 1e-6
    normed = arr / norm
    return pd.DataFrame(normed, columns=_SCORE_NAMES, index=scores.index)


def select_score(
    scores: pd.DataFrame,
) -> tuple[np.ndarray, np.ndarray]:
    """Select the score with the greatest absolute magnitude for each row.

    Args:
        scores: Row-normalised score DataFrame (output of
            :func:`normalize_scores`).

    Returns:
        Tuple ``(best_idx, best_score)`` where:

        * ``best_idx``   – integer index (0, 1, or 2) of the winning score
          per row.
        * ``best_score`` – the value of the winning score per row.
    """
    arr = scores[_SCORE_NAMES].values.astype(float)
    best_idx = np.argmax(np.abs(arr), axis=1)
    best_score = arr[np.arange(len(arr)), best_idx]
...
def apply_positions(
    ...
    *,
    ...
    """Translate selected scores into trading positions.

    For each row:

    1. If ``|best_score| < score_threshold`` → ``position = 0``.
    2. Otherwise:
       * ``base_signal = tanh(signal_v2_raw)``
       * Standard mode: ``position = base_signal * sign(best_score)``
       * Score-weighting mode: ``position = base_signal * best_score``

    Implemented with fully vectorised numpy operations.

    Args:
        df: Test DataFrame.  Must contain ``signal_v2_raw``.
        best_idx: Integer index of the winning score per row.
        best_score: Value of the winning score per row.
        score_threshold: Minimum ``|best_score|`` magnitude to take a
            position.
        use_score_weighting: If True, weight position by score magnitude
            instead of applying only direction.

    Returns:
        Tuple of three 1-D numpy arrays (all length = ``len(df)``):

        * ``positions``         – the final position array.
        * ``best_score_values`` – the chosen score value per row (0 where
          inactive).
    ...
    if "signal_v2_raw" not in df.columns:
        logger.warning("apply_positions: 'signal_v2_raw' missing; all positions = 0")
        return positions, best_score_values, chosen_score_idx

    base_signal = np.tanh(df["signal_v2_raw"].fillna(0.0).values.astype(float))

    active = np.abs(best_score) >= score_threshold
    chosen_score_idx[active] = best_idx[active]
    best_score_values[active] = best_score[active]

    if use_score_weighting:
        positions[active] = base_signal[active] * best_score[active]
    else:
        direction = np.sign(best_score[active])
...
def _fold_metrics(
    ...
    """Compute fold-level performance metrics for Regime V7.2.
...
def walk_forward(
    ...
    *,
    ...
    """Regime-V7.2 walk-forward: interaction-based score pipeline.
    ...
    3. Build interaction scores on train and test (using train stats).
    4. Apply row-wise normalisation to scores.
    5. Evaluate scores on **train only** (log diagnostics).
    6. Select highest-magnitude score per test row.
    7. Apply threshold and compute positions.
    8. Compute fold-level metrics.
    ...
    for i in range(2, len(years)):
        ...
        if test_valid.empty:
            logger.warning(
                "REGIME V7.2 [year=%d]: no valid test rows; skipping fold",
                test_year,
            )
            continue
        ...
        try:
            ...
            logger.warning(
                "REGIME V7.2 [year=%d]: missing train columns (%s); skipping",
                test_year,
                exc,
            )
            continue
        ...
        # ------------------------------------------------------------------
        try:
            train_scores_raw = build_scores(train_df, train_stats)
        except ValueError as exc:
            logger.warning(
                "REGIME V7.2 [year=%d]: build_scores failed on train (%s); skipping",
                test_year,
                exc,
            )
            continue
        ...
        try:
            test_scores_raw = build_scores(test_valid, train_stats)
        except ValueError as exc:
            logger.warning(
                "REGIME V7.2 [year=%d]: build_scores failed on test (%s); skipping",
                test_year,
                exc,
            )
            continue
        ...
        # ------------------------------------------------------------------
        # Step 3: Row-wise normalisation
        # ------------------------------------------------------------------
        train_scores = normalize_scores(train_scores_raw)
        test_scores = normalize_scores(test_scores_raw)

        # ------------------------------------------------------------------
        # Step 4: Evaluate scores on training data (diagnostics only)
        # ------------------------------------------------------------------
        logger.info("REGIME V7.2 [year=%d] — training score diagnostics:", test_year)
        ...
        # ------------------------------------------------------------------
        # Step 5: Select and apply scores to test rows
        # ------------------------------------------------------------------
        best_idx, best_score = select_score(test_scores)
        positions, best_score_values, chosen_score_idx = apply_positions(
            test_valid,
            best_idx,
            best_score,
        ...
        if np.isnan(returns_arr).all():
            logger.warning(
                "REGIME V7.2 [year=%d]: all returns NaN; skipping", test_year
            )
            continue
        ...
        coverage = m["coverage"]
        logger.info(
            "REGIME V7.2 FOLD | year=%d | n=%5d | mean=%+.6f"
    ...
    if not fold_rows:
        logger.warning(
            "REGIME V7.2: no valid folds produced (min_n=%d,"
            " score_threshold=%.4f)",
            min_n,
            score_threshold,
        )
...
def log_fold_results(fold_df: pd.DataFrame) -> None:
    ...
    Args:
        fold_df: DataFrame returned by :func:`walk_forward`.
    """
    logger.info("=== REGIME V7.2 — PER-FOLD RESULTS ===")
    if fold_df.empty:
        logger.warning("REGIME V7.2: no fold results to display")
        return
...
def log_final_summary(
    ...
    """Log the consolidated final summary of the Regime V7.2 pipeline.
    ...
    sep = "=" * 70
    logger.info(sep)
    logger.info("=== REGIME V7.2 — FINAL SUMMARY ===")
    logger.info(sep)

    if fold_df.empty:
        logger.warning("REGIME V7.2 SUMMARY: no results")
        return
...
# ---------------------------------------------------------------------------
# Module-level CLI (direct execution: python experiments/regime_v7_2.py)
# ---------------------------------------------------------------------------

def main(argv: list[str] | None = None) -> None:
    """Run the Regime V7.2 interaction-based score pipeline.
    ...
    require_columns(df, _REQUIRED_COLS, context="regime_v7_2.main")
